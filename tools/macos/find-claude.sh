#!/usr/bin/env bash
# find-claude.sh -- locate the claude binary across common macOS install layouts.
#
# Prints the resolved path on stdout. Exits 0 on success, 127 if not found.
# bash 3.2-safe (stock macOS /bin/bash is the frozen 3.2.57); no features
# newer than 3.2, no external deps beyond the BSD coreutils macOS ships.
#
# Probe order (first hit wins) -- mirrors tools/linux/find-claude.sh so the
# two platforms keep an identical discovery contract:
#   1. command -v claude          # PATH
#   2. $(npm config get prefix)/bin/claude
#   3. /opt/homebrew/bin/claude   # Apple Silicon Homebrew (the common case)
#   4. /usr/local/bin/claude      # Intel Homebrew + classic /usr/local
#   5. ~/.nvm/versions/node/*/bin/claude   (latest version)
#   6. fnm default alias (XDG path AND macOS "Library/Application Support")
#   7. asdf which claude
...
find_claude() {
    ...
    # 3-4. Homebrew (Apple Silicon first, then Intel/classic /usr/local).
    # On macOS this is the dominant install path, so it sits high.
    local p
    ...
    # 6. fnm. Linux defaults FNM_DIR to ~/.local/share/fnm; macOS defaults it
    # to ~/Library/Application Support/fnm. Probe both so a single contract
    # covers either layout (this is the one real macOS-vs-Linux divergence).
    local fnm_default
    for fnm_default in \
        "${HOME:-}/.local/share/fnm/aliases/default/bin/claude" \
        "${HOME:-}/Library/Application Support/fnm/aliases/default/bin/claude"; do
